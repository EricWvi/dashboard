...
fn render_endpoints_module(endpoints: &[FrontendEndpoint]) -> String {
    ...
    source.push_str("  authenticated: boolean;\n");
    source.push_str("};\n\n");
...
///
/// All operations include the `Onlyquant-Token` auth header when the endpoint is authenticated,
/// reading the token from `localStorage` at call time.
fn render_client_module(endpoints: &[FrontendEndpoint]) -> String {
    ...
    source.push_str("    headers: buildHeaders(endpoint.hasJsonBody, endpoint.authenticated),\n");
    source.push_str("  };\n\n");
    ...
    source.push_str("  }\n\n");
    source.push_str("  if (authenticated) {\n");
    source.push_str("    const token = localStorage.getItem(\"oqAuthToken\");\n");
    source.push_str("    if (token !== null) {\n");
    source.push_str("      headers[\"Onlyquant-Token\"] = token;\n");
    source.push_str("    }\n");
    source.push_str("  }\n\n");
...
fn render_index_module() -> String {
    ...
    source.push_str("export * from \"./auth.js\";\n");
    source.push_str("export * from \"./client.js\";\n");
    source.push_str("export * from \"./collection.js\";\n");
...
fn contract_module_for_type(type_name: &str) -> &'static str {
    match type_name {
        ...
        | "ListAllTodosRequest"
        | "ListAllTodosResponse"
        | "ListTodayTodosRequest"
        | "ListTodayTodosResponse"
        | "PlanTodayRequest"
        | "PlanTodayResponse" => "collection",
        "DeleteMediaRequest" | "DeleteMediaResponse" => "media",
        "ListEntriesRequest"
        | "ListEntriesResponse"
        | "CreateEntryRequest"
        | "CreateEntryResponse"
        ...
        | "UnbookmarkEntryRequest"
        | "UnbookmarkEntryResponse" => "entry",
        ...
        | "ClickBookmarkRequest"
        | "ClickBookmarkResponse" => "bookmark",
        "CreateTagsRequest" | "CreateTagsResponse" | "ListTagsRequest" | "ListTagsResponse"
        | "DeleteTagRequest" | "DeleteTagResponse" => "tag",
        "CreateTiptapRequest"
        | "CreateTiptapResponse"
        | "GetTiptapRequest"
        | "GetTiptapResponse"
        | "UpdateTiptapRequest"
        | "UpdateTiptapResponse"
        | "ListTiptapHistoryRequest"
        | "ListTiptapHistoryResponse"
        | "RestoreTiptapHistoryRequest"
        | "RestoreTiptapHistoryResponse"
        | "CreateQuickNoteRequest"
        | "CreateQuickNoteResponse"
        | "ListQuickNotesRequest"
        | "ListQuickNotesResponse"
        | "UpdateQuickNoteRequest"
        | "UpdateQuickNoteResponse"
        | "DeleteQuickNoteRequest"
...
fn render_http_method(endpoint: &FrontendEndpoint) -> &'static str {
    match endpoint.method {
        only_contracts::FrontendHttpMethod::Get => "GET",
        only_contracts::FrontendHttpMethod::Post => "POST",
        only_contracts::FrontendHttpMethod::Put => "PUT",
        only_contracts::FrontendHttpMethod::Delete => "DELETE",
    }
}
...
mod tests {
    ...
    use only_contracts::frontend_endpoints;
    use pretty_assertions::assert_eq;
    use std::fs;
    use tempfile::TempDir;

    /// Verifies the generated endpoint manifest includes the authenticated flag.
    #[test]
    fn renders_endpoint_manifest_with_authenticated_flag() {
        let module = render_endpoints_module(frontend_endpoints());

        assert!(module.contains("authenticated: true"));
        assert!(module.contains("authenticated: boolean"));
    }
    ...
    fn renders_endpoint_manifest_with_update_path_params() {
        ...
        assert!(module.contains("updateCollection"));
        assert!(module.contains("pathTemplate: \"/api/collections/{id}\""));
        assert!(module.contains("wireName: \"id\""));
    }

    /// Verifies the generated client attaches the auth token for authenticated endpoints.
    #[test]
    fn renders_client_with_auth_header_logic() {
        let module = render_client_module(frontend_endpoints());

        assert!(module.contains("Onlyquant-Token"));
        assert!(module.contains("localStorage.getItem(\"oqAuthToken\")"));
        assert!(module.contains("buildHeaders(endpoint.hasJsonBody, endpoint.authenticated)"));
    }
    ...
    /// Verifies the root index re-exports all domain modules.
    #[test]
    fn renders_index_with_all_domain_modules() {
        let module = render_index_module();

        assert!(module.contains("./auth.js"));
        assert!(module.contains("./collection.js"));
        assert!(module.contains("./media.js"));
