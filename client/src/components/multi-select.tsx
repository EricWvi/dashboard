...
import { Separator, Button, Badge, Popover, PopoverContent, PopoverTrigger, Command, CommandEmpty, CommandGroup, CommandInput, CommandItem, CommandList, CommandSeparator } from "@only/ui";
...
interface MultiSelectProps
  ...
  /**
   * If true, allows creating new options by typing in the search input.
   * Shows a "Create tag" option when search doesn't match existing options.
   * Optional, defaults to false.
   */
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
  (
    ...
    const getAllOptions = React.useCallback((): MultiSelectOption[] => {
      if (internalOptions.length === 0) return [];
      let allOptions: MultiSelectOption[];
      if (isGroupedOptions(internalOptions)) {
        allOptions = internalOptions.flatMap((group) => group.options);
      } else {
        allOptions = internalOptions;
      }
    ...
    }, [internalOptions, deduplicateOptions, isGroupedOptions]);
    ...
    const filteredOptions = React.useMemo(() => {
      if (!searchable || !searchValue) return internalOptions;
      if (internalOptions.length === 0) return [];
      if (isGroupedOptions(internalOptions)) {
    ...
    }, [internalOptions, searchValue, searchable, isGroupedOptions]);
    ...
    const createNewTag = (tagName: string) => {
      if (disabled || !allowCreateNew) return;
      ...
      // Add the new option to internal options
      if (isGroupedOptions(internalOptions)) {
        // For grouped options, add to the first group
        const updatedOptions = [...internalOptions];
        if (updatedOptions.length > 0) {
          updatedOptions[0] = {
            ...updatedOptions[0],
            options: [...updatedOptions[0].options, newOption],
          };
        } else {
        ...
        // For flat options, just add to the array
        setInternalOptions([
          ...(internalOptions as MultiSelectOption[]),
          newOption,
        ]);
      }
      ...
      // Clear search and optionally close popover
      setSearchValue("");
      if (closeOnSelect) {
        setIsPopoverOpen(false);
      }
    ...
    // Sync internalOptions with external options prop
    React.useEffect(() => {
