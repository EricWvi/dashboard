...
import type { Entry, EntryField, Tag, TagField } from "./model";
...
export class SqliteJournalDatabase implements IJournalDatabase {
  ...
  async getTag(id: string): Promise<Tag | undefined> {
    return (
      (await invoke<Tag | null>("journal_get_tag", { id })) ?? undefined
    );
  }
  ...
  async getTiptap(id: string): Promise<TiptapV2 | undefined> {
    return (
      (await invoke<TiptapV2 | null>("journal_get_tiptap", { id })) ??
      undefined
    );
  }
