...
2. **Explain "Why", not "What"**: Use comments to explain design rationale, business logic constraints, or non-obvious trade-offs. Avoid simply repeating the function or variable name in prose.
...
- Crate names are prefixed with `only-`. For example, the `core` folder's crate is named `only-core`
...
- Use only-logging wrapper macros instead of `tracing` macros. Use `only_logging::clock::now_local` instead of `OffsetDateTime::now_local()`.
...
# Frontend
- run backend: `task run:web-backend`
