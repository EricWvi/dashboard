...
Card sorting uses the same `DropZone` architecture as folders: cards are draggable-only display rows, while invisible 8px zones inserted before/between/after cards handle drop sensing and display a centered 2px insertion line when active. This maps each zone directly to a deterministic `generateKeyBetween(prevOrder, nextOrder)` result. Zones reject drops when the dragged card id matches either adjacent id to avoid no-op self-adjacent inserts.
